...
function calculateBlackWhiteRatio(binaryImage) {
    ...
    for (let i = 0; i < numPixels; i++) {
        ...
        if (pixelValue === 0) {
            blackPixels++;
        } else if (pixelValue === 255) { // 如果像素值为 255，则认为是白色像素
            whitePixels++;
        }
    }
    ...
    return `B：${(blackRatio * 100).toFixed(2)}%\nW：${(whiteRatio * 100).toFixed(2)}%`;
}

function displayImages() {
    let canvasOriginal = document.getElementById('canvasOriginal');
    let canvasGray = document.getElementById('canvasGray');
    let canvasBinaryTraditional = document.getElementById('canvasBinaryTraditional');
    ...
    contextOriginal.drawImage(originalImage, 0, 0, canvasOriginal.width, canvasOriginal.height);
    ...
    // 计算二值化图像的黑白像素比例
    let blackWhiteRatioTraditional = calculateBlackWhiteRatio(binaryTraditionalImage);
    let blackWhiteRatioOtsu = calculateBlackWhiteRatio(binaryOtsuImage);

    // 在页面上显示黑白像素比例
    document.getElementById('blackWhiteRatioTraditional').innerText = blackWhiteRatioTraditional;
    document.getElementById('blackWhiteRatioOtsu').innerText = blackWhiteRatioOtsu;
}
...
document.getElementById('downloadButton').addEventListener('click', function() {
    if (originalImage && grayImage && binaryTraditionalImage && binaryOtsuImage) {
        downloadImages();
    } else {
